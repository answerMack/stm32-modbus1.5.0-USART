...
eMBErrorCode eMBInit( eMBMode eMode, UCHAR ucSlaveAddress, UCHAR ucPort, ULONG ulBaudRate, eMBParity eParity )
{
    ...
    else
    {
        ...
        case MB_RTU:
            ...
            peMBFrameReceiveCur = eMBRTUReceive;/*modbus报文接受函数*/
        ...
        if( eStatus == MB_ENOERR )
        {
            if( !xMBPortEventInit(  ) )
            {
                /* port dependent event module initalization failed.端口初始化失败 */
                eStatus = MB_EPORTERR;
            }
            else
            {
...
eMBErrorCode eMBEnable( void )
{
    ...
    if( eMBState == STATE_DISABLED )
    {
        /* Activate the protocol stack.激活协议栈 */
        pvMBFrameStartCur(  );
...
eMBErrorCode eMBPoll( void )
{
    ...
    int             i;
    eMBErrorCode    eStatus = MB_ENOERR;//modbus 协议栈错误码
    ...
    if( xMBPortEventGet( &eEvent ) == TRUE )//查询事件
    {
        switch ( eEvent )
        {
        case EV_READY:
        ...
        case EV_FRAME_RECEIVED: /*接收到一帧数据，此事件发生*/
            ...
            {
                /* Check if the frame is for us. If not ignore the frame. *//*判断接收到的报文数据是否可接受，如果是，处理报文数据*/
